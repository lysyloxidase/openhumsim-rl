...
Run from the repository root:
    ...
    PYTHONPATH=src python examples/dashboard_server.py
...
def _source_fingerprint() -> dict[str, Any]:
    ...
    package_sources = sorted((ROOT / "src" / "openhumsim_rl").glob("*.py"))
    source_paths = [ROOT / "examples" / "dashboard_server.py", *package_sources]
    files = [
        {
    ...
    def step(
        ...
        *,
        ...
        with self.lock:
            ...
            _, reward, self.terminated, self.truncated, self.info = self.env.step(vector)
...
def main() -> int:
    ...
    try:
        DashboardHandler.allowed_hosts = frozenset(
            ...
            for value in [args.host, *args.allowed_host]
        )
    except ValueError as exc:
